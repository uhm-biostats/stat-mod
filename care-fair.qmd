...
In the researcher-initiated workflow, once Notices are applied to data via a project, an Indigenous community can then replace Notices with Labels. Unlike Notices, Labels *do* articulate the rights and protocols of Indigenous peoples. There are many different kinds of Labels, indeed there needs to be many to express rights and interests. Only communities can apply Labels, they are out of the purview of researchers and settler institutions. It should of course be noted that researchers can belong to the community from which the data they work with originate. Such scholars may have roles in both applying Notices and applying Labels if they are given that authority by their community. 

The Labels are organized into Traditional Knowledge (TK) and Bicultural (BC), and within each of those groups there are Provenance, Protocol, and Permission Labels. Here are example Labels from each category. Text is quoted directly from the hyperlinked sources because we (really Andy speaking here) do not have authority to change the language
