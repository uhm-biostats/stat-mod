...
```
Error in library(dplyr) : there is no package called ‘xyz’
```
...
Now try a few other operators out. Filter the `toy_data` so that we extract rows where `x2` is less than or equal to 5 but greater than 4.4.
...
```{r}
mutate(clean_toy_data, log_dbh = log(dbh))
```
...
# this passes the output of c(1, 4, 8) into sum()
sum(c(1, 4, 8))
...
final_dat <- select(toy_data, -tree_ID) |> 
    filter(species == "sp1") |> 
    rename(dsla = x1, dbh = x2)

final_dat
```

That's the `data.frame` we want and hopefully a more clear, less error-prone way of getting there. One thing to note, that sometimes trips people up, is that after passing `toy_data` into `select`, we don't see any names of objects again. But remember, the *output* of `select` is being piped into `filter` as the first argument, and the output of `filter` is getting pipped into `rename` as the first argument.  If you got that, great! If not, here is one more way to think about it
...
final_dat <- select(toy_data, -tree_ID) |> 
    filter(__hidden-output-of-select__, species == "sp1") |> 
    rename(__hidden_output-of-filter__, dsla = x1, dbh = x2)
```
...
- add a column for log DBH and call it `log_dbg_cm`
...
Let's join the three real data sets we'll be working with! Let's call the product of joining all the data `all_dat`. Don't forget to also remove rows where `Island` is `NA`. Let's also re-arrange the columns so they are ordered like this: 
...
Printing `data.frame`s, even using `head`, can result in some hard-to-read output. The deeper you go in wrangling data you will also find some inconsistencies in the design choices (made decades ago) about how data.frames sometimes behave. For all those reasons, there is an alternative option that tries to retain all the good things about `data.frames`, be more-or-less interoperable with `data.frame`s, but also give us some improvements. This is the `tibble`. To work with `tibble`s we'll need the *tibble* package. To see the printing benefit of `tibble`s let's turn our monstrous `all_dat` object into a `tibble` and have a look:
...
We don't even need to worry about using `head`, we get more information about each column (kind of like `str`), and columns don't run off the screen or wrap around the console, some are just not printed and we get a message about which ones were not printed.
