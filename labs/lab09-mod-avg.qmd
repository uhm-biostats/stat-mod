...
Because we will ultimately be building models with these data and averaging those models, we need to be very conservative in our handling of missing values. If our explanatory variables have differences in terms of which variables are missing, model averaging will fail. So a final step in our data preparation workflow needs to be `filter`ing out any rows that contain `NA` for the explanatory variables we'd like to use. Plan which explanatory variables you'd like to build models with and then use `filter(!is.na(x1) & !is.na(x2) & !is.na(x3) & ... so on)` for all your intended explanatory variables.
