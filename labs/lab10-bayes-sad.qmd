...
$$
\begin{aligned}
\log(\lambda_{ij}) &= b_0 + b_1 \text{NDVI}_j + b_2 \text{hii}_j + b_3 I_{\text{native, } i} + b_4 I_{\text{native, } i} \text{NDVI}_j \\
y_{ij} &\sim \text{Pois}(\lambda_{ij})
\end{aligned}
$$

$I_{\text{native, } i}$ is an indicator function that equals 1 if species $i$ is native, and equals 0 otherwise. In this way $b_3 I_{\text{native, } i}$ represents the additional effect of being native relative to $b_0$ which is the effect of being non-native (recorded as "alien" in the *OpenNahele* data). Similarly, $b_4 I_{\text{native, } i} \text{NDVI}_j$ is the additional slope term added for species that are native.
...
\log(\lambda_{ij}) &= b_0 + b_1 \text{NDVI}_j + b_2 \text{hii}_j + b_3 I_{\text{native, } i} + b_4 I_{\text{native, } i} \text{NDVI}_j + b_{\text{sp} ~i} + b_{\text{site} ~j} \\
y_{ij} &\sim \text{Pois}(\lambda_{ij})
\end{aligned}
$$
...
- $b_2 \text{hii}_j$
- $b_3 I_{\text{native, } i}$
- $b_4 I_{\text{native, } i} \text{NDVI}_j$
