...
gpll <- ggplot(ll, aes(a0, a1, z = loglik)) +
    geom_tile(aes(fill = loglik)) +
    scale_fill_viridis_c() +
    geom_contour(color = "black") +
    xlab(expression(hat(a[0]))) +
...
`optim` traverses the space to find the maximum
...
Where do we get the `llfun` function? <br>
We have to make it



```{r}
llfun <- function(a, y, elev) {
    dbinom(x = y, size = 1, 
           prob = 1 / (1 + exp(-(a[1] + a[2] * elev))), 
           log = TRUE) |> 
        sum()
}

# find the maximum 
llmax <- optim(par = c(-3, 0.005), 
               fn = llfun,
               y = y, elev = elev, 
               control = list(fnscale = -1))
```
...
<-- the max likelihood estimates


<-- value of log lik function at max
